...
void HelloTriangleApplication::createRenderPass() {
    ...
    // multi-sampled images cannot be presented directly. First need to resolve them to a regular image
    ...
    colourAttachmentResolve.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    ...
    depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE; // depth data wont be used after drawing has finished
...
void HelloTriangleApplication::createGraphicsPipeline() {
    ...
    depthStencil.depthCompareOp = VK_COMPARE_OP_LESS; // lower depth = close, so the depth of new fragments should be less

    // if you want to only keep fragments that fall within the specified depth range
    ...
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional - if wanted a new graphics pipeline derived from an existing pipeline
...
void HelloTriangleApplication::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        // but the same depth image can be used by all of them because only a single subpasss
...
void HelloTriangleApplication::recordCommandBuffer(VkCommandBuffer cmdBuffer, uint32_t imageIndex) {
    ...
    // the initial value at each point in the depth buffer should be the futhest possible depth, which is 1.0
...
void HelloTriangleApplication::createDescriptorSets() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        descriptorWrites[0].dstBinding = 0; // uniform buffer binding index is 0, layout(binding = 0)
        descriptorWrites[0].dstArrayElement = 0; // descriptors can be arrays, we are not using an array so the index is 0
...
void HelloTriangleApplication::createTextureImage() {
    ...
    std::cout << "Texture loaded successfully, imageSize: " << imageSize << ", mipLevels: " << mipLevels << std::endl;
    ...
    // intend to use the texture image as both the source and destination of a transfer
...
void HelloTriangleApplication::transitionImageLayout(VkImage image, uint32_t mipLevels, VkFormat format,
    ...
    memoryBarrier.subresourceRange.baseArrayLayer = 0; // our image is not an array
    ...
    */
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED
        ...
        // the depth buffer will be read from to perform depth tests
        // to see if a fragment is visibile
        ...
        sourceStage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT; // when writing happens
        destinationStage = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT; // when reading happens
    } else {
...
VkImageView
HelloTriangleApplication::createImageView(VkImage image, VkFormat format, VkImageAspectFlagBits aspectFlags,
                                          uint32_t mipLevels) {
...
void HelloTriangleApplication::createDepthResources() {
    ...
    // unlike texture image, we don't necessarily need a specific format, because we won't be directly accessing the texels
    // just needs to have a reasonable accuracy, at least 24 bits is common
...
void HelloTriangleApplication::generateMipmaps(VkImage image,
    ...
    for (uint32_t i = 1; i < nMipLevels; i++) {
        ...
        // source mip level is i-1 and the destimation mip level is 1
        VkImageBlit blit{};
        // the 3D region the data will be blitted from
        ...
        // the region the data will be blitted to
        blit.dstOffsets[0] = {0, 0, 0};
        blit.dstOffsets[1] = {mipWidth > 1 ? mipWidth / 2 : 1, // (each mip level is half the size of the previous one)
        ...
        // blitting between different levels of the same image
        ...
        // waits on the current blit command to finish
        vkCmdPipelineBarrier(commandBuffer,
                             VK_PIPELINE_STAGE_TRANSFER_BIT,
        ...
        // if texture is not a square then one of the mip dimensions could reach 0 while the other dim is still halving
        if (mipWidth > 1) {
            mipWidth /= 2;
        }
        if (mipHeight) {
    ...
    // to VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT ... since this want handled by the loop
    vkCmdPipelineBarrier(commandBuffer,
                         VK_PIPELINE_STAGE_TRANSFER_BIT,
                         VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                         dependencyFlags,
...
void HelloTriangleApplication::createColourResources() {
